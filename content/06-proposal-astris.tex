...
Fortunately our protocol description does not touch on cache invalidation. However, the protocol bears a distinct resemblance to the Helios v4 protocol. This is due to the fact that many constructs within Helios are perfectly suited to the application in this protocol. There are also significant differences in both the way elections are set up, voters registered and how the public bulletin board is constructed. Therefore, \emph{Astris} --- one of the daughters of Helios in Greek mythology --- seemed a fitting name.
...
With this in mind, the aims of the Astris protocol are:

\begin{itemize}
    \item Create a voting protocol that is correct, secret, fair, robust, receipt-free, individually and universally verifiable, universally auditable and honours eligibility.
    \item Create a voting protocol with no single points of trust, minimizing trust requirements.
    \item Create a voting protocol which maximizes auditability and transparency.
\end{itemize}
