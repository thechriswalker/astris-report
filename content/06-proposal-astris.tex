...
\begin{enumerate}
    ...
    \item Each Trustee Creates their \dtref{TrusteeSetup}{dt:setup:trustee} from the initial data and returns it to the Authority.
          \begin{enumerate}
              ...
              \item For each $c \in C$, calculate the exponent $e = g^c mod p$. Let $E$ be the ordered set of all the exponents for each $c$. These exponents act as a commitment to the coefficients, which the trustee does not reveal.
              \item Create a \dtref{Signature}{dt:elgamal:pok} $S$ over $(i, E, P_{sig}, P_{enc}, Z_{enc})$ by creating a message: \itodo{message}
              \item The trustee returns $(i, E, P_{sig}, P_{enc}, Z_{enc}, S)$ to the Authority and keeps hold of the secret keys and coefficients.
          \end{enumerate}
    \item The Registrar creates their \dtref{RegistrarSetup}{dt:setup:registrar} data from the initial data and returns it to the Authority.
          \begin{enumerate}
              ...
              \item Create a \dtref{Signature}{dt:elgamal:sig} $S$ over $(P_{sig}, U_r)$
...
\todo{There is an addition validation that can be done at this point? Or is it after the next bit.}
...
\begin{enumerate}
    \item Each trustee $i$:
          \begin{enumerate}
              ...
              \item Creates a \dtref{ProofOfKnowledge}{dt:elgamal:pok} of the secret key $x$.
...
The chain should only accept blocks where both the proof of knowledge and the signatures are valid, and we have not seen the trustee's block yet.
...
\begin{enumerate}
    ...
    \item The voter then combines this data into the \nameref{dt:payload:reg} and publishes the block.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item For each candidate, the voter:
          \begin{enumerate}
              \item Creates a \dtref{CipherText}{dt:elgamal:ct} of the value $0$ or $1$ as the vote (or not) for that candidate. We use Exponential ElGamal for this as it is additively homomorphic, so the actual value enciphered is $n = g^v mod p$ where $v \in (0,1)$. This encryption requires a random value $r$, which is available to the voter. This breaks the receipt-free property. See \autoref{ch:analysis:impl} for a full discussion of the implications of this.
              \item Creates a \dtref{ZeroKnowledgeProof}{dt:elgamal:zkp} to prove that the ciphertext correctly encrypts a $0$ or a $1$ (but not which). \itodo{ZKP method}
          \end{enumerate}
    \item The voter now creates a final \dtref{ZeroKnowledgeProof}{dt:elgamal:zkp} proving that the sum of the votes is at most $x$.
    ...
    \item The voter combines this data along with their voter ID into a \dtref{PayloadCastVote}{dt:payload:cast} and submits it to the chain.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item The trustee produces a \dtref{ZeroKnowledgeProof}{dt:elgamal:zkp} of correct decryption.
...
We allow all trustees to add their blocks. Each additional block after the first $x$ should also be able to be combined with a $x - 1$ other partial decryption results to produce the \emph{same} tally. Therefore, the more we have the more confident we can be in the final result.
