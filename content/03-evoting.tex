...
As the result is only a single number, it is the number of \emph{1} votes and the count of voters registered minus the result will give the number of \emph{0} votes.
...
Therefore, while the trust requirements are objectively lower due to an enhancement in eternal secrecy, the author would argue that the reduction would be is minimal and that this proposal has very similar trust requirements to Belenios.
...
The voting system is designed to be run on the HyperLedger Fabric (\surl{https://www.hyperledger.org/projects/fabric}) blockchain. This system is a permissioned blockchain network, where the public will not have direct access to the ledger data however the voting protocol requires the all organizations involved in the network make their ledger data available which satisfies the auditability property.
...
%         \hline
%         \textbf{Trustless-Gajek} & \ALL              & \ALL          & \ALL              & \ALL           & \ALL             & \ALL                   & \ALL                      & \ALL                           & \ALL                          & \ALL               & \ALL                                     & \NO                   \\
%         \hline
%     \end{tabular}
...
% During the research phase of this report, the author read about many voting systems, some of which did not differ enough to warrant a section of their own. However, some did contain specific technologies or ideas that are worthy of note and so we will briefly look at some of these other systems.

% In \cite{yuPlatformindependentSecureBlockchainBased2018} the voter secrecy is enhanced by the use of \gls{lrs} on votes. A \gls{lrs} is a form of digital signature that allows a group of public key holders to sign data in a way that does not allow any entity to ascertain which signer was associated with the data and also, to ensure that we can detect if the same signer attempts to sign twice. This is an interesting approach as covers the secrecy property excellently, whilst simultaneously preventing double-voting. However, in this system it precludes repeat voting and requires much interactivity during the voting process.
